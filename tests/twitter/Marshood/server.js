const express = require('express')
const app = express();
var path = require("path");
var http = require('http');
const fs = require("fs");// to read json file 
//npm init  --y
//npm i yargs
//npm install express
//
//
// DB

const url = "mongodb+srv://Marshood:raMHdQuDOBxwrcss@cluster0.ifcjp.mongodb.net/twitter";
const mongoose = require('mongoose');
mongoose.connect(url, { useNewUrlParser: true, useUnifiedTopology: true });

var bodyParser = require('body-parser');
const { isNull } = require('util');
app.use(bodyParser.json());       // to support JSON-encoded bodies
app.use(bodyParser.urlencoded({     // to support URL-encoded bodies
  extended: true
}));
app.use(express.static('public'));
var urlencodeParser= bodyParser.urlencoded({extended: false});
    app.use(express.static(path.join(__dirname,"")));
app.use(express.static(path.join(__dirname,"public")));

 const UserT = mongoose.model('User', {
    name: String,
    img: String,
 });

 const OnlineUser= mongoose.model('OnlineUser', {
    name: String,
    img: String,
 });
 const Twitt= mongoose.model('Twitt', {
    Twitt: String,
    img: String,
    name: String 
 });



 app.put('/AddUser', function (req, res) {
    console.log("AddUser...")
    var UserName = req.body.Name;
    var UserIMG = req.body.IMG;
    
    const User = new UserT({ name: UserName, img: UserIMG });
    const CurrentUser = new OnlineUser({ name: UserName, img: UserIMG });

    OnlineUser.deleteOne({ }, function (err) {
        if (err) return handleError(err);
        // deleted at most one tank document
      });
    User.save().then(() => console.log('wellcome'));
    CurrentUser.save().then(() => console.log('wellcome'));

    res.send("mainScreen.html");
  
    });


    app.get('/GetUsersOnline',function(req,res) {
        OnlineUser.find({}, function (err, doc) {
            res.send(doc)
          })
       })



       
// const User = new Twitt({ Twitt: "hola test hola test hola test", img: "data:image/jpeg;base64,",name: "Test1" });
// User.save().then(() => console.log('wellcome'));

    app.get('/getTwitts',function(req,res) {
        Twitt.find({}, function (err, doc) {
            res.send(doc)
          })
       })


       app.put('/postTwitt', function (req, res) {
        console.log("postTwitt...")
        var Twittext = req.body.Twitt;
        var img = req.body.img;
        var name = req.body.name;
      
        const TwittNew = new Twitt({ Twitt: Twittext, img: img, name:name});
     
        TwittNew.save().then(() => console.log('Saved...'));

        
        res.send("mainScreen.html");
      
        });
    
        
    app.listen(8080, () => { console.log("App is Listening") })
